...
class PlanExecutor:
    ...
    def run(self, plan_run: PlanRun) -> PlanRun:
        ...
        while True:
            failed_dependency = _first_step_with_failed_dependency(plan_run.steps)
            if failed_dependency is not None:
                return self._mark_failed(
                    plan_run=plan_run,
                    error=(
                        f"Plan step {failed_dependency.step_id} cannot run because "
                        "a dependency failed."
                    ),
                )
    ...
    def _execute_step(self, *, plan_run: PlanRun, step: PlanStep) -> None:
        while True:
            step.status = "running"
            plan_run.workflow_status = "running"
            ...
            _persist_step_observation(step=step, observation=observation)
            ...
            if observation.requires_user:
                ...
                step.observation = _attach_observation_hitl_metadata(
            ...
            if not observation.retryable:
                ...
                self._mark_failed(
                    plan_run=plan_run,
                    error=observation.error or observation.result_summary,
                )
                return

            step.status = "retrying"
            _transition(plan_run, "tool_error_retryable")
            plan_run.error = observation.error or observation.result_summary
    ...
    def _synthesize_success(self, plan_run: PlanRun) -> PlanRun:
        observations = [
            ...
            if step.status == "succeeded" and step.observation is not None
        ]
...
def _first_step_with_failed_dependency(steps: Sequence[PlanStep]) -> PlanStep | None:
    step_by_id = {step.step_id: step for step in steps}
    for step in steps:
        if step.status != "pending":
            continue
        if any(
            step_by_id[dependency_id].status in {"failed", "cancelled"}
...
def _persist_step_observation(*, step: PlanStep, observation: ToolObservation) -> None:
