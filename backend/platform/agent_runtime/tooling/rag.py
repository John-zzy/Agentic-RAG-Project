...
from backend.platform.agent_runtime.core.validation import validate_tool_input
    ...
    *,
    ...
    retrieval_trace.update(
        _evidence_metadata(
            final_decision=final_decision,
            citations=citations,
            metadata=_outcome_metadata(outcome),
        )
    )
...
def _evidence_metadata(
    *,
    ...
    nested_trace = _nested_retrieval_trace(metadata)
    adopted_citations = _metadata_list(
        metadata.get("adopted_citations")
        or nested_trace.get("adopted_citations")
        or citations
    )
    answer_mode = (
        metadata.get("answer_mode")
        or nested_trace.get("answer_mode")
        or _answer_mode_from_decision(final_decision)
    )
    no_hit_fallback = (
        metadata.get("no_hit_fallback")
        if "no_hit_fallback" in metadata
        else nested_trace.get("no_hit_fallback", final_decision == RAG_FINAL_DECISION_NO_EVIDENCE)
    )
    return {
        "adopted_citations": adopted_citations,
        "answer_mode": answer_mode,
        "no_hit_fallback": bool(no_hit_fallback),
    }


def _nested_retrieval_trace(metadata: Mapping[str, Any]) -> Mapping[str, Any]:
    trace = metadata.get("retrieval_trace") or metadata.get("document_retrieval_trace")
...
def _answer_mode_from_decision(final_decision: str) -> str:
    if final_decision == RAG_FINAL_DECISION_ANSWER:
        return "evidence_answer"
    if final_decision == RAG_FINAL_DECISION_ASK_USER:
        return "follow_up"
    if final_decision in {RAG_FINAL_DECISION_NO_EVIDENCE, RAG_FINAL_DECISION_FAILED}:
        return "fallback"
    return final_decision


def _outcome_metadata(outcome: AgenticRetrievalOutcome) -> dict[str, Any]:
